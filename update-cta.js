...
const BASE = '/data/.openclaw/workspace/trailofchina';
...
function generateCTA(page) {
  return `      customToursCTA={
        <section className="mb-10">
          <div className="rounded-2xl border-2 border-[#ebe4d8] bg-[#faf8f4] p-6 sm:p-8 text-center shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-center gap-4 text-xs text-[var(--muted)]">
              <span>${page.bullets[0]}</span>
              <span>${page.bullets[1]}</span>
              <span>${page.bullets[2]}</span>
            </div>
          </div>
        </section>
      }`;
...
for (const page of pages) {
  ...
  const searchLine = `const searchQuery = encodeURIComponent('${page.searchQuery}');\n\n`;
  content = content.replace(/export default function/, searchLine + 'export default function');

  // Insert CTA block before the final `    />` of AttractionPage component.
  // The component ends with `    />` then newline then `  );` then newline then `}`
  const finalClose = /\n(    \/>\s*\n  \);\s*\n\})$/;
  const match = content.match(finalClose);
  if (match) {
    const cta = generateCTA(page);
    content = content.slice(0, match.index) + '\n' + cta + match[0];
  } else {
    // Fallback: just before the last `    />`
    const idx = content.lastIndexOf('\n    />');
    if (idx !== -1) {
      const cta = generateCTA(page);
      content = content.slice(0, idx) + '\n' + cta + content.slice(idx);
    }
  }
