...
// ── POI Data (same as Leaflet version) ───────────────────────────
...
// ── Component ────────────────────────────────────────────────────
export default function ShanghaiMapGL() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="bg-white border-b border-[#ebe4d8] sticky top-0 z-[70]">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-[#1a3a4a]">Shanghai Map <span className="text-xs font-normal text-[#64748b] ml-1">(WebGL)</span></h1>
      ...
      {/* Map */}
      <div className="flex-1" style={{ minHeight: 'calc(100vh - 110px)' }}>
        <Map
          ref={mapRef}
          initialViewState={{
            longitude: 121.47,
          ...
          minZoom={11}
          maxZoom={18}
          maxBounds={[{ lng: 120.8, lat: 30.9 }, { lng: 122.2, lat: 31.6 }]}
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{ width: isSelected ? 36 : 30, height: isSelected ? 36 : 30 }}
                >
                  {/* Pulse ring */}
                  <div
                    className="absolute inset-0 rounded-full animate-ping opacity-20"
                    style={{ backgroundColor: cfg.color, animationDuration: '2s' }}
                  />
                  {/* Dot */}
                  <div
          ...
          {selectedPoi && (
            ...
            >
              <div style={{ minWidth: 200, maxWidth: 260 }}>
                <img
                  src={`https://picsum.photos/seed/${selectedPoi.imageSeed}/400/200`}
                  alt={selectedPoi.name}
                  style={{ width: '100%', height: 100, objectFit: 'cover', borderRadius: 8, marginBottom: 8 }}
                />
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontSize: 16 }}>{categoryConfig[selectedPoi.category].emoji}</span>
                  <strong style={{ color: '#1a3a4a', fontSize: 14 }}>{selectedPoi.name}</strong>
                </div>
                <p style={{ fontSize: 12, color: '#64748b', margin: '4px 0' }}>{selectedPoi.hook}</p>
                {selectedPoi.tag && <span style={{ fontSize: 11, color: '#af5d32', fontWeight: 600 }}>{selectedPoi.tag}</span>}
                <a href={selectedPoi.href} style={{ display: 'block', marginTop: 8, fontSize: 13, fontWeight: 600, color: '#af5d32' }}>Explore →</a>
              </div>
            </Popup>
          )}
        </Map>
