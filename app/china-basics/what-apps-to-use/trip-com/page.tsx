    ...
    <div className="min-h-screen bg-gradient-to-b from-[#faf8f4] to-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        <section className="relative mt-6 mb-12">
          <ChineseWatermark character="携" />
          <p className="text-[#af5d32] font-medium text-sm tracking-wide uppercase mb-2">China Basics › Apps</p>
          <h1 className="font-serif text-4xl sm:text-5xl font-bold text-[var(--foreground)] leading-tight mb-2">
            Trip.com — Your All-in-One China Travel App
          </h1>
          <p className="font-serif text-2xl text-[#af5d32] opacity-70 mb-4">携程旅行</p>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {capabilities.map((c) => (
              <div
                key={c.label}
                className="flex flex-col items-center gap-1 py-3 px-2 rounded-lg bg-white shadow-sm border border-[#e8e0d4]"
              >
                <span className="text-2xl">{c.icon}</span>
                <span className="text-xs font-medium text-[var(--foreground)] text-center leading-tight">{c.label}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-14">
          <h2 className="font-serif text-2xl font-bold text-[var(--foreground)] mb-6">What Trip.com Can Do</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {overviewCards.map((card) => (
              <div
                key={card.title}
                className="p-5 rounded-xl bg-white border border-[#e8e0d4] shadow-sm hover:shadow-md transition-shadow"
              >
                <span className="text-3xl mb-3 block">{card.icon}</span>
                <h3 className="font-serif font-bold text-lg text-[var(--foreground)] mb-1">{card.title}</h3>
        ...
        {/* 1. High-Speed Trains */}
        <section className="mb-14">
          <h2 className="font-serif text-2xl font-bold text-[var(--foreground)] mb-4">🚄 High-Speed Trains</h2>
          ...
          <h3 className="font-serif font-bold text-lg text-[var(--foreground)] mb-3">How to Book</h3>
          <ol className="space-y-3 mb-6">
            {[
              ...
              <li key={i} className="flex items-start gap-3">
                <span className="flex-shrink-0 w-7 h-7 rounded-full bg-[#af5d32] text-white text-sm font-bold flex items-center justify-center">
                  {i + 1}
                </span>
                <span className="text-[var(--muted)] leading-relaxed pt-0.5">{step}</span>
              </li>
            ))}
          </ol>

          <div className="border-l-4 border-[#af5d32] bg-white p-4 rounded-r-lg">
        ...
        {/* 2. Hotels */}
        <section className="mb-14">
          <h2 className="font-serif text-2xl font-bold text-[var(--foreground)] mb-4">🏨 Hotels &amp; Accommodation</h2>
          <p className="text-[var(--muted)] leading-relaxed mb-3">
        ...
        {/* 3. Flights */}
        <section className="mb-14">
          <h2 className="font-serif text-2xl font-bold text-[var(--foreground)] mb-4">✈️ Flights</h2>
          <p className="text-[var(--muted)] leading-relaxed mb-3">
        ...
        <section className="mb-14">
          <h2 className="font-serif text-2xl font-bold text-[var(--foreground)] mb-4">🎫 Attraction Tickets</h2>
          <p className="text-[var(--muted)] leading-relaxed mb-3">
        ...
        {/* 5. eSIM */}
        <section className="mb-14">
          <h2 className="font-serif text-2xl font-bold text-[var(--foreground)] mb-4">📱 eSIM</h2>
          <p className="text-[var(--muted)] leading-relaxed mb-3">
          ...
          <h3 className="font-serif font-bold text-lg text-[var(--foreground)] mb-3">How to Set Up</h3>
          <ol className="space-y-3 mb-6">
            {[
        ...
        {/* 6. Airport Transfers */}
        <section className="mb-14">
          <h2 className="font-serif text-2xl font-bold text-[var(--foreground)] mb-4">🚕 Airport Transfers &amp; Car Rental</h2>
          <p className="text-[var(--muted)] leading-relaxed mb-3">
        ...
        {/* ── PRICE COMPARISON ── */}
        <section className="mb-14">
          <h2 className="font-serif text-2xl font-bold text-[var(--foreground)] mb-6">Is the Markup Worth It?</h2>
          <p className="text-[var(--muted)] leading-relaxed mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            {/* Trains comparison */}
            <div className="p-5 rounded-xl bg-white border border-[#e8e0d4]">
              <h3 className="font-serif font-bold text-lg text-[var(--foreground)] mb-3">🚄 Train Tickets</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between py-2 border-b border-[#e8e0d4]">
                  <span className="text-[var(--muted)]">12306 (official)</span>
                  <span className="font-medium text-[var(--foreground)]">¥553 (Beijing→Shanghai, 2nd class)</span>
                </div>
                <div className="flex justify-between py-2 border-b border-[#e8e0d4]">
                  <span className="text-[var(--muted)]">Trip.com</span>
                  <span className="font-medium text-[var(--foreground)]">¥553 + ¥10–30 service fee</span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-[var(--muted)]">Markup</span>
                  <span className="font-medium text-[#af5d32]">~2–5% (¥10–30)</span>
                </div>
              </div>
              <p className="text-xs text-[var(--muted)] mt-3">12306 requires Chinese payment method and Chinese phone number. Trip.com accepts foreign cards + English interface.</p>
            </div>

            {/* Hotels comparison */}
            <div className="p-5 rounded-xl bg-white border border-[#e8e0d4]">
              <h3 className="font-serif font-bold text-lg text-[var(--foreground)] mb-3">🏨 Hotels</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between py-2 border-b border-[#e8e0d4]">
                  <span className="text-[var(--muted)]">Booking.com</span>
                  <span className="font-medium text-[var(--foreground)]">Varies — often similar pricing</span>
                </div>
                <div className="flex justify-between py-2 border-b border-[#e8e0d4]">
                  <span className="text-[var(--muted)]">Trip.com</span>
                  <span className="font-medium text-[var(--foreground)]">Often cheaper for Chinese hotels</span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-[var(--muted)]">Edge</span>
                  <span className="font-medium text-[#af5d32]">Trip.com (deeper local inventory)</span>
                </div>
              </div>
              <p className="text-xs text-[var(--muted)] mt-3">Trip.com has direct relationships with Chinese hotels that Booking.com may not. Both accept foreign cards.</p>
            </div>
          </div>
        ...
        {/* ── PRO TIPS ── */}
        <section className="mb-14">
          <h2 className="font-serif text-2xl font-bold text-[var(--foreground)] mb-6">Know Before You Go</h2>
          <div className="space-y-3">
            {proTips.map((tip, i) => (
              <p key={i} className="text-[var(--muted)] leading-relaxed pl-2">
                <span className="text-[#af5d32] mr-2">•</span>
                {tip}
              </p>
            ))}
          </div>
        ...
        {/* ── FAQ ── */}
        <section className="mb-14">
          <h2 className="font-serif text-2xl font-bold text-[var(--foreground)] mb-6">Frequently Asked Questions</h2>
          <div className="space-y-2">
            {faqs.map((faq, i) => (
              <details
                key={i}
                className="group rounded-xl bg-white border border-[#e8e0d4] overflow-hidden"
              >
                <summary className="flex items-center justify-between cursor-pointer p-5 font-serif font-bold text-[var(--foreground)] hover:text-[#af5d32] transition-colors list-none">
                  <span>{faq.question}</span>
                  <span className="ml-2 text-[#af5d32] transition-transform group-open:rotate-45 text-xl leading-none">+</span>
                </summary>
        ...
        <section className="mb-14 text-center">
          <div className="p-8 sm:p-10 rounded-2xl bg-gradient-to-br from-[#faf8f4] to-white border border-[#e8e0d4]">
            <h2 className="font-serif text-2xl sm:text-3xl font-bold text-[var(--foreground)] mb-3">Get Trip.com</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <a
                ...
                className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-[var(--foreground)] text-white rounded-lg font-medium hover:opacity-90 transition-opacity"
              >
                🍎 App Store
              </a>
              <a
            ...
            <p className="text-xs text-[var(--muted)] mt-4">Available on App Store and Google Play</p>
          </div>
        </section>

        {/* ── RELATED ── */}
        <section className="mb-10">
          <h2 className="font-serif text-2xl font-bold text-[var(--foreground)] mb-4">Related Pages</h2>
          <div className="flex flex-wrap gap-2">
            {relatedLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-4 py-2 rounded-full bg-white border border-[#e8e0d4] text-sm font-medium text-[var(--foreground)] hover:border-[#af5d32] hover:text-[#af5d32] transition-colors"
              >
                {link.label}
              </Link>
            ))}
