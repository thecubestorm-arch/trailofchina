...
import { Layers, List, Map, Search } from "lucide-react";
import type { CityHubConfig, FilterTag } from "./types";
...
const getItemCategory = (itemType: CityHubConfig["allItems"][number]["type"]): POICategory | null => {
...
const poiMarkerIcon = (
  ...
  const pills = visibleCategories
    .map((category) => {
      ...
      const isHighlighted =
        highlightedCategory === null || highlightedCategory === category;
...
export default function CityMapView({
  ...
  const [zoom, setZoom] = useState(config.mapZoom);
  ...
  const expandedMaxBounds = useMemo(() => {
    const allAttractionCoords = config.unifiedLocations.map(
      (loc) => [loc.lat, loc.lng] as [number, number]
    );
    return [
      [
        Math.min(config.maxBounds[0][0], ...allAttractionCoords.map((c) => c[0])),
        Math.min(config.maxBounds[0][1], ...allAttractionCoords.map((c) => c[1])),
      ] as [number, number],
      [
        Math.max(config.maxBounds[1][0], ...allAttractionCoords.map((c) => c[0])),
        Math.max(config.maxBounds[1][1], ...allAttractionCoords.map((c) => c[1])),
      ] as [number, number],
    ] as [[number, number], [number, number]];
  }, [config.maxBounds, config.unifiedLocations]);
  ...
  }, [activeCategories, activeView, config.markers, matchedItems]);
    ...
    <div className="space-y-4">
      ...
      >
        ...
        >
          ...
          <div ref={listRef} className="flex-1 space-y-4 overflow-y-auto p-3">
            {filteredItems.length === 0 && (
              <div className="py-12 text-center">
                <Search className="mx-auto mb-3 text-[#64748b]" size={32} />
                <p className="mb-1 text-sm font-semibold text-[#1a3a4a]">
                  No matches
                </p>
            ...
            {groupPoisByCategory.map((group) => {
              ...
              return (
                <div key={group.category}>
                  <h3 className="mb-2 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-[#1a3a4a]">
                    <span>{cfg.emoji}</span>
                    {cfg.label}
                  </h3>
                  <div className="space-y-2">
                    {group.pois.map((poi) => (
                      <Link
                        key={poi.id}
                        href={poi.href}
                        className={`group flex items-start gap-3 rounded-xl border-l-[3px] border-y border-r p-2.5 transition-all ${
                          hoveredItem === poi.markerId
                            ? "border-l-[#af5d32] border-y-[#ebe4d8] border-r-[#ebe4d8] bg-[#faf4ea]"
                      ...
                      >
                        <div className="relative h-14 w-14 flex-shrink-0 overflow-hidden rounded-lg bg-[#ebe4d8]">
                          {poi.imageSeed ? (
                            <Image
                        ...
                        <div className="min-w-0 flex-1">
                          <h4 className="truncate text-sm font-semibold text-[#1a3a4a] transition-colors group-hover:text-[#af5d32]">
                            {poi.name}
                          </h4>
                          {poi.tag && (
                            <span
        ...
        >
          <MapContainer
            ...
            zoom={config.mapZoom}
            className="city-hub-map z-0"
            bounds={expandedMaxBounds}
            minZoom={8}
            maxZoom={18}
          ...
          >
            ...
            {layerVisibility.metroStations &&
              zoom >= 12 &&
              config.metroLines.flatMap((line) =>
            ...
            {filteredMarkers.map((marker) => (
              ...
              >
                <Popup>
                  <Link
                    href={marker.primaryItem?.href ?? `/destinations/${config.slug}`}
        ...
        <style jsx global>{`
          ...
          .city-hub-map .leaflet-top,
          .city-hub-map .leaflet-bottom {
            z-index: 2;
          }
        `}</style>
      </div>
